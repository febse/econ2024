...
Strict equality generally makes sense for strings and integers, but not for floating point numbers! Due to the way floating point numbers are represented in the computer, you may get unexpected results when comparing them. This is because floating point numbers are not stored exactly in the computer. For example, the number 0.1 cannot be represented exactly in binary, so it is stored as an approximation. This can lead to unexpected results when comparing floating point numbers. For example, you may get a result like this:
...
This is because `sqrt(2)^2` is computed with a finite precision and is not equal to 2. When printing the number in the console you may not see the difference because the print function formats the number. To compare floating point numbers, you should use the `all.equal` function, which takes into account the fact that floating point numbers are not stored exactly in the computer.
