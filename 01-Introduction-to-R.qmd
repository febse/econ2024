...
R Studio provides a lost of useful shortcuts that can speed up your workflow.
We will mostly use the following shortcuts in R Studio:
...
Vectors can only hold data of the same type. Basically this
...
Pay attention to the warning message. It is telling you that the shorter vector is being recycled to match the length of the longer vector. Most of the time, this is not what you want. It is better to be explicit about what you want to happen, so don't rely on recycling with vectors of different lengths.
...
When comparing floating point numbers, you should be careful. Due to the way floating point numbers are represented in the computer, you may get unexpected results when comparing them. This is because floating point numbers are not stored exactly in the computer. For example, the number 0.1 cannot be represented exactly in binary, so it is stored as an approximation. This can lead to unexpected results when comparing floating point numbers. For example, you may get a result like this:

```{r}
sqrt(2)^2 == 2
```

Mathematically, $\sqrt(2)^2$ is exactlyequal to 2, but the comparison returns `FALSE`.

This is because `sqrt(2)^2` is not exactly equal to 2. To compare floating point numbers, you should use the `all.equal` function, which takes into account the fact that floating point numbers are not stored exactly in the computer.
:::


## Factors

Factors are used to represent categorical data. They are stored as integers and have labels associated with them. Factors are important in statistical modeling and are commonly used in plotting functions. Factors are not strings, they are integers with labels.
...
```
Factors have also some safeguards. If you try to perform an operation that is not defined for factors, R will issue a warning. For example, you cannot meaningfully add a number to a factor.
Note that this will only raise a warning, not an error.
...
A thing to note is that R is (mostly) a functional programming language and functions generally do not modify their arguments. This means that if you pass a vector to a function and the function modifies the vector, the original vector will not be changed. This is different from many other languages, where functions can modify their arguments. The reason for this is that arguments are copied when they are passed to a function. This is done to avoid side effects and make the code easier to reason about.
