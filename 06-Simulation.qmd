...
and remember that the average deviation of a bunch of values from their average
is zero. 
...
The above expression uses the fact that the $y_i$ are uncorrelated given $x$ and have the same variance $\sigma^2$ (homoscedasticity) and the fact that the weights $w_i$ are fixed (not random) under the condition $x$. It furthermore uses the fact that the variance of a sum of _uncorrelated_ random variables is the sum of the variances of these random variables.
...
So we can denote the denomitator as 
